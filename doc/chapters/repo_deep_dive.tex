...
Running the \bashinline{make} command produces a simulator executable in the verilator directory.
...
Some of the generators are accelerators; they are meant to be implemented as add-on processors to main CPUs.
For example, the \nameref{sec:SHA3_Accelerators_Generator} accelerator is meant to be implemented with a \nameref{sec:Rocket_Chip} as its main CPU.\@
...
% TODO: Explain the difference between vectors and matrices in terms of computation,
% particularly when the difference comes to building an accelerator specifically for them.
...
NVDLA, short for NVIDIA Deep Learning Accelerator, is a domain-specific \gls{soc}, designed for deep learning.
...
This means that a single package can be composed of not only Rocket CPUs, but \emph{also} custom accelerators, co-processors, or completely independent cores, or even a mix of all three.
...
\nocite{sha3Paper}
\nocite{sha3Github}
The SHA3 generator is a \glslink{parameterize}{parameterized} accelerator compute unit, like the \nameref{sec:Gemmini_Generator} and \nameref{sec:Hwacha}.
...
All of this is done in a ``set and forget'' way, where the main processor gives the accelerator the required information and the accelerator is free to run on its own.
...
\begin{blackbox}
  ...
  If you are defining a completely new design (a new accelerator for instance), then you can separate this definition out to a different generator directory entirely.
  You can also track that as a submodule of Chipyard, have Chipyard import, and dynamically handle your new project as a dependency of the one that generates the CPU design.
\end{blackbox}
